...
class BookModel extends Model
{
    ...
    {
        $builder = $this->table('booking');
        $builder->select('booking.id as id, booking.address as address, paymentMethod, booking.userId as uId, serviceId, booking.created_at as date, serviceId, users.picture as picture, categoryValue as category, locationValue as location, booking.status as status, price, users.name as name');
        $builder->join('service', 'service.id = serviceId');
